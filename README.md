...
Docker é uma plataforma que permite automatizar o processo de implantação de aplicativos em containers, oferecendo uma abordagem para "empacotar" um aplicativo com todas as suas dependências em um contêiner virtual. Já o Docker Compose é uma ferramenta que facilita a definição e execução de aplicativos Docker compostos por vários contêineres (ou seja, serve como um orquestrador).

Para esse projeto, optei por implementar os seguintes serviços em containers:
- Banco de Dados PostgreSQL
- Apache ZooKeeper (Um serviço que é uma dependência para o funcionamento do Kafka em um ambiente distribuído)
- Apache Kafka

Há diversas vantagens em se utilizar o Docker. Vejo a utilização de containers como uma boa prática de desenvolvimento, já que eles são capazes de <b>isolar as dependências</b>, garantindo que o ambiente de execução seja consistente em diferentes máquinas e ambientes, e também por sua <b>portabilidade</b>, que permite que o contêiner seja executado em qualquer lugar que suporte Docker, independentemente do sistema operacional ou da infraestrutura subjacente. Pensando na possibilidade de alguém quiser executar o projeto, sem o uso do Docker haveria um problema, pois sendo o banco de dados local, seria impossível a execução do Spring Boot, assim como seria necessário executar um servidor Kafka com as mesmas configurações de tópico, grupo e portas do projeto para que o Produtor, Consumidor e o Listener configurados no projeto pudessem realizar suas ações. 

Com relação ao Springboot e ao Vue.js, optei por não incluí-los em containers por alguns motivos plausíveis:
- <b>Flexibilidade de ambiente de desenvolvimento:</b> manter o Spring Boot e o Vue.js fora dos containers me permitiu ter mais flexibilidade em termos de configuração e ajustes no ambiente de desenvolvimento local. Dessa forma, por exemplo, era possível ajustar rapidamente configurações sem a necessidade de recriar ou modificar os containers.
- <b>Desempenho:</b> executar o Spring Boot e o Vue.js diretamente no host durante o desenvolvimento oferece melhor desempenho do que executá-los dentro de containers Docker. Isso foi particularmente relevante para mim já que a inicialização rápida e o tempo de resposta foram importantes para a produtividade do desenvolvimento. Em containers, seria necessário a todo momento recriar o `jar` do Spring ou instalar as dependências do Vue.js e copiar todos os arquivos para dentro do container, um processo que prolonga significativamente o tempo para a execução.
- <b>Facilidade de Depuração:</b> manter o Spring Boot e o Vue.js fora dos containers facilitou a depuração durante o desenvolvimento, já que pude usar ferramentas de depuração padrão das IDEs e navegadores utilizadas diretamente no código fonte, sem a necessidade de configurar ou conectar-se aos containers. 

Já a utilização do Docker Compose veio da necessidade de uma orquestração simples dos três contêineres que estão sendo utilizados no projeto e que possuem uma relação de dependência entre eles. Outra necessidade que havia e com o Docker Compose pude resolver, é o de garantir que um determinado serviço seja iniciado antes de outro, ou seja, o Docker Compose garante que os serviços sejam iniciados na ordem correta e que tenham acesso uns aos outros conforme necessário. Esse era um problema que eu enfrentava com relação o Apache Kafka, já que o serviço necessita de que o ZooKeeper esteja sendo executado antes dele, caso contrário, o Kafka apresentará diversos erros. Assim como, para iniciar o Spring Boot, é necessário que o contêiner do PostgreSQL, ZooKeeper e Kafka estejam em execução.
